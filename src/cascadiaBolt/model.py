from cascadia.depthcharge.encoders import PeakEncoder, FloatEncoder
from cascadia.depthcharge.transformers import SpectrumTransformerEncoder, PeptideTransformerDecoder
...
class AugmentedSpec2Pep(pl.LightningModule):
    ...
    # ── CascadiaBolt Change 3: predict_step with torch.no_grad() ──────────────
    # Original code called torch.set_grad_enabled(True) inside predict_step,
    # which re-enabled the autograd engine and forced PyTorch to build and
    # store a full computation graph for every decode step. That graph is never
    # used (no .backward() call during inference), so it wasted ~1.5–2× memory
    # and ~30–50 % of forward-pass time purely on bookkeeping.
    # Fix: drop the set_grad_enabled call and wrap the decode loop in
    # torch.no_grad() so autograd is unconditionally disabled regardless of
    # whatever Lightning version is in use.
    def predict_step(self, batch, *args):
        ...
        with torch.no_grad():
            for i in range(len(sequences[0]) + 1):
                ...
                aa_conf = torch.cat(
                    [aa_conf, torch.reshape(next_aa_scores[range(len(next_aas)), next_aas], (-1, 1))],
                    dim=1,
                )
                cur_sequences = torch.cat(
                    [cur_sequences, torch.reshape(next_aas, (-1, 1))],
                    dim=1,
                )
