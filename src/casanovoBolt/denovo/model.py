...
class Spec2Pep(pl.LightningModule):
    ...
    **kwargs : Dict
    ...
    def beam_search_decode(
        ...
        scores[:, :1, :, :] = einops.repeat(pred, "B L V -> B L V S", S=beam).to(scores.dtype)
        ...
        try:
            ...
            for step in range(0, self.max_peptide_len):
                ...
                if torch.any(active_beams):
                    ...
                    scores[active_beams, : step + 2, :] = active_scores.to(scores.dtype)
    ...
    def _finish_beams(
        ...
        if torch.any(beams_to_check):
            ...
            # CasanovoBolt: GPU-native vectorized mass calculation.
            # Replaces a Python for-loop + CPU tokenizer call + two GPU↔CPU
            # transfers that previously occurred on every decode step.
            #
            # Strategy: gather token sequences for all active beams at once,
            # zero-out stop tokens (token_masses[0]==0 so they contribute
            # nothing), then sum residue masses from the on-device buffer.
            #   neutral_mass = Σ residue_masses + H₂O (18.010565 Da)
            #   precursor_mz = neutral_mass / z + 1.007276 (proton)
            seq_block = tokens[idx, : step + 1].clone()  # (n, step+1)
            last_is_stop = seq_block[:, step] == self.stop_token
            seq_block[last_is_stop, step] = 0  # zero → contributes 0 mass

            if seq_block.shape[1] > 0:
                masses_per_token = token_masses[seq_block]           # (n, step+1), float64
                valid_mask = (seq_block != 0).to(token_masses.dtype)  # exclude padding
                recalc_neutral_masses = (
                    (masses_per_token * valid_mask).sum(dim=1) + 18.010565
                )
                charges_d = charges_to_check.double()
                ...
                recalc_neutral_masses = torch.zeros(
    ...
    def _get_topk_beams(
        ...
        # CasanovoBolt: use empty_like instead of clone() — all meaningful
        # positions are overwritten below so the clone is unnecessary and
        # wastes memory bandwidth (scores tensor is ~3M floats per batch).
        tokens_new = torch.empty_like(tokens)
        tokens_new[:, :step, :] = einops.rearrange(
            prev_tokens[b_idx, :, 0, s_idx_flat], "(B S) L -> B L S", S=beam
        )
        tokens_new[:, step, :] = v_idx

        scores_new = torch.empty_like(scores)  # CasanovoBolt: empty_like, not clone
