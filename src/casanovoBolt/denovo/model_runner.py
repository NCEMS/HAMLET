...
class ModelRunner:
    ...
    def initialize_trainer(self, train: bool) -> None:
        ...
        """
        # CasanovoBolt: allow multi-GPU for inference when devices > 1 is set.
        infer_devices = 1
    ...
    def initialize_model(self, train: bool, db_search: bool = False) -> None:
        ...
        # CasanovoBolt: optionally compile the encoder for ~10-30% faster inference.
        if getattr(self.config, "compile_encoder", False) and not train:
            try:
                ...
                logger.info("CasanovoBolt: encoder compiled with torch.compile (reduce-overhead).")
            except Exception as exc:
                logger.warning(
                    "CasanovoBolt: torch.compile failed, running without: %s", exc
                )
